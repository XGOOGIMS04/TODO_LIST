...
confirmBtn.addEventListener("click", (event) => {
    ...
    modalText.value = ""; // 확인 버튼을 누르면 텍스트 입력창 초기화
});
...
for(i = 0; i < newItem.length; i++){ // 새로고침 후 localStrage에 불러와서 다시 그림. for문으로 하나씩 newItem에서 꺼내옴
    ...
    // 새로고침 하면 전에 했던 체크가 저장되어있음
    reCheckBox.addEventListener("change", function () {
        let onChecked = Number(reCheckBox.getAttribute("data-index")); // 클릭된 체크박스의 인덱스 번호를 가져와서 변수에 담음
        newItem[onChecked].checked = reCheckBox.checked; // newItem 배열에서 그 번호에 해당하는 항목의 checked를 현재 체크박스 실제 상태로 업데이트함
        localStorage.setItem("newItem_obj", JSON.stringify(newItem)); // 체크상태 변경시 localStorage 업데이트
    });
    reCheckBox.setAttribute("data-index", i); // reCheckBox에 "data-index" 라는 이름으로 i 값 저장
}
...
//------ CLEAR 버튼 누르면 체크리스트 내용 다 삭제됨(동기부여까지 )
